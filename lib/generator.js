...
Generator.buildSpecialPages = function () {
  var pagedPosts = index.getPagedPosts(5),
    ...
    tags = index.getTags(),
    processed = 0,
    maxProcessed = 5 + pagedPosts.length + Object.keys(tags).length
  ;
    ...
    function(resolve, reject) {
      ...
      fs.remove(config.directories.htdocs + '/index*', function (err) {
        for (page = 0; page < pagedPosts.length; page ++) {
          ...
          fs.writeFile(config.directories.htdocs + '/' + curPageObj.currentUrl, Mustache.render(templates.index, curPageObj, partials), checkProcessed);
        }
      });
      ...
      fs.remove(config.directories.htdocs + '/tagged', function (err) {
        Object.keys(tags).map(function (key) {
          ...
          fs.writeFile(config.directories.htdocs + '/tagged/' + tags[key].id + '/index.html', Mustache.render(templates.index, tags[key], partials), checkProcessed);
        });
      });

      fs.writeFile(config.directories.htdocs + '/404.html', Mustache.render(templates.four, {
      ...
      fs.writeFile(config.directories.htdocs + '/rss.json', JSON.stringify(RssJs(index.getPosts(20), dateFormat(index.pubDate, 'ddd, dd mmm yyyy hh:MM:ss o'))), checkProcessed);
