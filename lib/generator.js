...
const webcomponents       = require('./helpers/webcomponents-from-html');
const slacked             = require('./models/slacked');
const jsonFeed            = require('./models/json-feed');
const jsonLd              = require('./models/json-ld');
...
const Generator = function(config) {
  ...
  if (config.specialFeatures.acceleratedmobilepages) {
    Handlebars.ampCss =
      Handlebars.ampCss
      || fs.readFileSync(path.join(Handlebars.themePath, '../css/amp.css'), 'utf8').replace(/\s*[\n\r]+\s*/g, '')
    ;
  }
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        let urls = {
          ...
          journal:    indexUrl(path + 'journal.ics'),
          ajax:       indexUrl(path + 'index.json'),
          gophermap:  indexUrl(path + 'gophermap')
        };
        let promises = [];
        ...
        for (page = 0; page < pagedPosts.length; page ++) {
          ...
          promises.push(fs.writeFileAsync(
            indexUrl(curPageObj.currentUrl).filename(),
  ...
  external.buildTagPages = function() {
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        promises.push(fs.writeFileAsync(
          indexUrl(config.htdocs.tag + '/index.html').filename(),
  ...
  external.buildCategoryPages = function() {
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        promises.push(fs.writeFileAsync(
          indexUrl(config.htdocs.category + '/index.html').filename(),
          Handlebars.compileExtra(Handlebars.themeTemplates.tagsHtml, {
            index: categoryPages,
            config: config
          }, Handlebars.themePartials)
