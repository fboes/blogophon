...
const webmentions    = require('./helpers/webmentions')(true);
...
const Generator = function(config) {
  ...
  Handlebars.getThemeTemplates(path.join(config.directories.currentTheme, 'templates'));

  if (config.specialFeatures.acceleratedmobilepages) {
    Handlebars.ampCss = Handlebars.ampCss || fs.readFileSync(path.join(Handlebars.themePath, '../css/amp.css'), 'utf8').replace(/\s*[\n\r]+\s*/g, '');
  }
  ...
  external.buildSingleArticle = function(post, noimages, force) {
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        if (config.specialFeatures.acceleratedmobilepages) {
          promises.push(fs.writeFileAsync(post.meta.urlObj.filename('amp'), Handlebars.compileExtra(Handlebars.themeTemplates.ampPostHtml, {
            post: post,
  ...
  external.buildArticleImages = function(post) {
    if (!post.meta.Url || !post.filename || post.attachmentDir) {
      return false;
    }
    // Target directory
    let sourceDirectory = post.attachmentDir; // Source directory
    let sourceGlob      = glob.sync(path.join(sourceDirectory, "*.{png,jpg,gif}"));
    ...
    return new Promise(
      function(resolve, reject) {
        let promises = sourceGlob.map(function(sourceFile) {
          let targetFile = sourceFile.replace(sourceReg, config.directories.htdocs + post.meta.Url); // TODO: This is madness
  ...
  external.copyAttachmentFiles = function(post) {
    ...
    // Target directory
    let sourceDirectory = post.attachmentDir; // Source directory
    let sourceGlob      = glob.sync(path.join(sourceDirectory, "!(*.png|*.jpg|*.gif|*.md)"));
    let sourceReg       = new RegExp(sourceDirectory);
      ...
      function(resolve, reject) {
        ...
        if (config.specialFeatures.rss || config.specialFeatures.facebookinstantarticles) {
          promises.push(fs.writeFileAsync( urls.rss.filename(), Handlebars.compileExtra(Handlebars.templates.rssXml, {
            ...
            title:       title
          }, {contentHtml: config.specialFeatures.facebookinstantarticles ? Handlebars.templates.facebookHtml : '{{{safeHtml}}}'})));
        }
        ...
        if (config.specialFeatures.teasersnippets) {
          promises.push(fs.writeFileAsync( urls.snippetHtml.filename(), Handlebars.compileExtra(Handlebars.themeTemplates.snippetHtml, {
        ...
        if (config.specialFeatures.icscalendar) {
          promises.push(fs.writeFileAsync( urls.ics.filename(), Handlebars.compileExtra(Handlebars.templates.calendarIcs, {
        ...
        if (config.specialFeatures.jsonrss) {
          promises.push(fs.writeFileAsync( urls.jsonrss.filename(), JSON.stringify(jsonRss(index.getPosts(20), index.pubDate, config, title, urls.jsonrss.absoluteUrl()), undefined, 2)));
        }
        if (config.specialFeatures.jsonforslack) {
          promises.push(fs.writeFileAsync( urls.slackjson.filename(), JSON.stringify(slacked(index.getPosts(3), index.pubDate, config, title), undefined, 2)));
        }
        ...
        if (config.specialFeatures.kml) {
          promises.push(fs.writeFileAsync( urls.networkKml.filename(), Handlebars.compileExtra(Handlebars.templates.networkKml, {
            config:      config,
            absoluteUrl: urls.networkKml.absoluteUrl(),
            title:       title,
            link:        urls.placesKml.absoluteUrl()
          })));
          promises.push(fs.writeFileAsync( urls.placesKml.filename(), Handlebars.compileExtra(Handlebars.templates.placesKml, {
            index:       index.getPosts(),
            config:      config,
        ...
        for (page = 0; page < pagedPosts.length; page ++) {
          ...
          promises.push(fs.writeFileAsync(indexUrl(curPageObj.currentUrl).filename(), Handlebars.compileExtra(Handlebars.themeTemplates.indexHtml, curPageObj, Handlebars.themePartials)));
  ...
  external.buildTagPages = function() {
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        promises.push(fs.writeFileAsync( indexUrl(config.htdocs.tag + '/index.html').filename(), Handlebars.compileExtra(Handlebars.themeTemplates.tagsHtml, {
          index: tagPages,
          config: config
        }, Handlebars.themePartials)));
  ...
  external.buildCategoryPages = function() {
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        promises.push(fs.writeFileAsync( indexUrl(config.htdocs.category + '/index.html').filename(), Handlebars.compileExtra(Handlebars.themeTemplates.tagsHtml, {
          index: categoryPages,
          config: config
        }, Handlebars.themePartials)));
