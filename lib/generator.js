 ...
 * @return {Promise} with first parameter being the number of files converted.
 */
    ...
    function(resolve, reject) {
     glob(config.directories.data + "/**/*.md", function (er, files) {
        ...
        for (i = 0; i < maxProcessed; i++) {
          PostReader(files[i]).then(checkProcessed);
        }
      });
    }
  );
    ...
    function(resolve, reject) {
      ...
      for (i = 0; i < maxProcessed; i++) {
        ...
        fs.writeFile(config.directories.htdocs + post.meta.Url + 'index.html', Mustache.render(templates.post, {
          post: post,
          config: config
        },partials),checkProcessed( post.meta.Url + 'index.html'));
      }
    }
  );
};
    ...
    function(resolve, reject) {
      ...
      fs.remove(config.directories.htdocs + '/index*', function (err) {
        for (page = 0; page < pagedPosts.length; page ++) {
          ...
          fs.writeFile(config.directories.htdocs + '/' + curPageObj.currentUrl, Mustache.render(templates.index, curPageObj, partials), checkProcessed('index file'));
        }
      });
      ...
      fs.remove(config.directories.htdocs + '/tagged', function (err) {
        Object.keys(tags).map(function (key) {
          ...
          fs.writeFile(config.directories.htdocs + '/tagged/' + tags[key].id + '/index.html', Mustache.render(templates.index, tags[key], partials), checkProcessed("/tagged/" + tags[key].id + '/index.html'));
        });
      });
