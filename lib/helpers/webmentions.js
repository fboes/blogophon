 ...
 * @param  {Boolean} console [description]
 * @param  {String}  ua      [description]
 * @return {Object}          [description]
 */
const webmentions = function(console = false, ua = 'Blogophon\'s Webmention Adapter') {
  const external = {};
  const internal = {};
  ...
  internal.sendMention = function(sourceUrl, targetUrl) {
    if (console) {
      console.log('Trying to send mention to ' + targetUrl);
    }
    return internal
   ...
   */
  internal.discoverEndpoint = function(targetUrl) {
    return new Promise(function(resolve, reject) {
      ...
      client.get(targetUrlObj, function(response) {
        let matches = [];
        ...
        if (!matches || !matches[1]) {
          ...
          response.on('end', function() {
            ...
            if (matches && matches[1].match(/href=/)) {
              resolve(internal.makeCompleteUrl(targetUrl, matches[1].replace(/^.*href="([^"]*)".*$/, '$1')));
            } else {
              reject(400);
            }
          });
