...
const webmentions = function(useConsole = false, ua = 'Blogophon\'s Webmention Adapter') {
   ...
   * @param  {String}  testUrl [description]
   * @return {Boolean}         [description]
   */
  internal.isExternalUrl = function(testUrl) {
    let success = testUrl.match(/^https?:\/\//);
    if (success) {
      const urlObj = url.parse(testUrl);
      return !urlObj.host.match(/(\.(wikipedia|amazon)\.|(example|jalopnik|play\.google)\.com)/);
    }
    return false;
  };
