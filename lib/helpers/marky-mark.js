...
const markyMark = function(string, rules) {
  ...
  internal.convertResult = function(html) {
    ...
    return html
      ...
      .replace(/(<blockquote)(>[\s\S]+?<\/blockquote>\s*)<p>\s*(<cite)(>[\s\S]+?<\/cite>)\s*<\/p>/g, (all, bq, bqend, cite, citeend) => {
        const matches = citeend.match(/href="(.+?)"/);
        if (matches) {
          bq += ' cite="' + matches[1] + '"';
        }
        return bq + ' class="with-cite"' + bqend + cite + ' class="blockquote"' + citeend;
      })
