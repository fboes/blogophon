...
const markyMark = function(string, rules) {
  ...
  internal.convertShell = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        ...
        .replace(/(\W)(--?[a-zA-Z0-9_-]+)(\W)/g, '$1<tt>$2</tt>$3')
        .replace(/(\W|>)(%[a-zA-Z0-9_-]+%)(\W)/g, '$1<var>$2</var>$3')
        .replace(/(\W|>)((?:\$)[a-zA-Z0-9_-]+)(\W)/g, '$1<var>$2</var>$3')
        .replace(/(\W)(&(?:lt|gt);)(\W)/g, '$1<samp>$2</samp>$3')
        .replace(/(^|\n)(\$)( .+?)/g, '$1<em>$2</em>$3')
      ;
    });
  };
  ...
  internal.convertSql = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        .replace(/(^|\b)(SELECT|UPDATE|INSERT|REPLACE|DELETE|IGNORE|DELAYED|QUICK|INTO|FROM|(?:LEFT |RIGHT |INNER |OUTER |CROSS )?JOIN|ON|DUPLICATE KEY|WHERE|BETWEEN|IS|NOT|X?OR|AND|AS|IN|LIKE|MATCH|REGEXP|CASE|UNIQUE|CREATE|DISTINCT|INDEX|ALTER|IF|EXISTS|VALUES?|SET|USE|LIMIT|ORDER BY|ASC|DESC|GROUP BY|HAVING)(\W)/g, '$1<b>$2</b>$3')
