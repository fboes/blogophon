...
const markyMark = function(string, rules) {
      ...
      switch (internal.mode) {
        ...
        case '<>':
          if (chunk.match(/<code class/)) {
            const lang = chunk.match(/(css|less|sass|html|xml|xslt|markdown|shell|bash|dos|yaml|ini|apacheconf)/);
            if (lang && lang[1]) {
              newMode = '<'+lang[1]+'>';
            } else {
              newMode = '<code>';
  ...
  internal.convertShell = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        .replace(/(^|\b)(set|echo|cd|rm|del|md|mkdir|cp|copy|mv|move|ren|touch|less|more|exit|time|sudo)(\b)/g, '$1<b>$2</b>$3')
