...
const markyMark = function(string, rules) {
  ...
  internal.convertSql = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        .replace(/(^|\b)(SELECT|UPDATE|INSERT|REPLACE|DELETE|INTO|FROM|(?:LEFT |RIGHT |INNER |OUTER |CROSS )?JOIN|ON|WHERE|BETWEEN|IS|NOT|X?OR|AND|AS|IN|LIKE|UNIQUE|CREATE|DISTINCT|INDEX|ALTER|EXISTS|VALUES|SET|USE|LIMIT|ORDER|GROUP BY|HAVING)(\W)/g, '$1<b>$2</b>$3')
        .replace(/(^|\b)([A-Z]+)(\()/g, '$1<b>$2</b>$3')
