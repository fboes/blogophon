...
const markyMark = function(string, rules) {
      ...
      switch (internal.mode) {
        ...
        case '<>':
          if (chunk.match(/<code class/)) {
            const lang = chunk.match(/(css|less|sass|html|xml|xslt|markdown|shell|bash|dos|yaml|ini|apacheconf|sql|abc)/);
            if (lang && lang[1]) {
              newMode = '<'+lang[1]+'>';
            } else {
              newMode = '<code>';
  ...
  internal.convertShell = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        ...
        .replace(/(\W)(--?[a-zA-Z0-9_-]+)(\W)/g, '$1<em>$2</em>$3')
  ...
  internal.convertSql = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        ...
        .replace(/(^|\b)(NULL|TRUE|FALSE|[\d.]+)(\W)/g, '$1<samp>$2</samp>$3') //?
        .replace(/(^|\b)([a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+)(\W)/g, '$1<samp>$2</samp>$3')
        .replace(/<\/(b|i)>(\s*)<\1>/, '$2')
      ;
    });
  };
  ...
  internal.convertResult = function(html) {
    ...
    return html
      ...
      .replace(/(<a[^>]+)(title)="(nomention|nofollow)"/g, function(all, link, attribute, value) {
        switch (value) {
          case 'nofollow':
            attribute = 'rel';
            break;
