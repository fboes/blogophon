import emojiConvert from './emoji-convert.js';
...
const markyMark = function(string, rules) {
  ...
  internal.pushChunk = function(chunk, newMode) {
    ...
    if (chunk) {
      switch (internal.mode) {
        ...
        case '<xml>':
        case '<xslt>':
          chunk = internal.convertXml(chunk);
          break;
        case '<javascript>':
          chunk = internal.convertJavascript(chunk);
          break;
        case '<markdown>':
        ...
        case '<axiom>':
          ...
          break;
        case '<metar>':
          chunk = internal.convertMetar(chunk);
          break;
        case '<>':
          if (chunk.match(/<code class/)) {
            const lang = chunk.match(/((?:css|less|sass)|html|(?:xml|xslt)|javascript|markdown|metar|(?:shell|bash|dos|batch)|yaml|ini|(?:apache|nginx)(?:conf)?|sql|abc|axiom|json)/);
   ...
   * @return {String}                         [description]
   */
  internal.convertGeneralCode = function(code, codeConverterFunction, breaker = /(\/\*[\S\s]+?\*\/|(?:\/\/|#).+?(?:\n|\r|$)|&quot;.*?&quot;|'[^']*?'|`[^`]*?`)/g, lineComment = '#') {
   ...
   * @return {String}        [description]
   */
  internal.convertCode = function(string, breaker) {
  ...
  internal.convertCss = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        ...
        .replace(/(^|\W)(@(?:[a-z]+))(\b)/g, '$1<i>$2</i>$3')
        .replace(/((?:^|\n)\s*)(.+?)(\s?:\s?)(.+?)(;)/g, (all, pre, property, inter, value, post) => {
          return pre
            + '<b>' + property + '</b>'
            + inter
            + value
              .replace(/(^|\W)([a-z-]+)(\()/g, '$1<b>$2</b>$3')
              .replace(/(^|\W)(#[a-zA-Z0-9]+)/g, '$1<em>$2</em>')
            + post;
        })
        .replace(/((?:^|\n)\s*)([^<{}>]+?)(\s?{)/g, '$1<i>$2</i>$3')
        .replace(/<b>(<var>.+?<\/var>)<\/b>/g, '$1')
      ;
    }, /(\/\*[\S\s]+?\*\/|(?:\/\/).+?[\n\r]|&quot;.*?&quot;|'[^']*?'|`[^`]*?`)/g);
  };
  ...
  internal.convertShell = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        .replace(/(^|\b)(setx?|echo|cd|rm|del|md|mkdir|cp|copy|mv|move|ren|touch|less|more|exit|time|sudo)(\W)/g, '$1<b>$2</b>$3')
        .replace(/(^|\b)(if|fi|then|case|esac|function|for|do|done)(\W)/g, '$1<i>$2</i>$3')
        .replace(/(\s)(--?[a-zA-Z0-9_-]+)/g, '$1<em>$2</em>')
        .replace(/(%[a-zA-Z0-9_-]+%|\$[a-zA-Z0-9_-]+|\$\{[^}]+\})/g, '<var>$1</var>')
        .replace(/(^|\n)(\S+)(=)/g, '$1<var>$2</var>$3')
        .replace(/(\W)((?:&(?:lt|gt|amp);)+)(\W)/g, '$1<i>$2</i>$3')
        .replace(/(^|\n)(\$ )([^\n]+)/g, '$1<em class="bang">$2</em><span class="cmd">$3</span>')
      ;
    });
  };

  /**
   * Convert JavaScript text node
   * @param  {String} string [description]
   * @return {String}        [description]
   */
  internal.convertJavascript = function(string) {
    return this
      .convertCode(string)
      .replace(/(^|\b)(document)(\W)/g, '$1<b>$2</b>$3')
      .replace(/(\.)(getAttribute|setAttribute|hasAttribute|getElementById|getElementsByTagName|querySelector(?:All)?|replace|match(?:es)?|test|addEventListener)(\()/g, '$1<b>$2</b>$3')
      .replace(/(=&gt;)/g, '<samp>$1</samp>')
    ;
  };
   ...
   * @return {String}        [description]
   */
  internal.convertXml = function(string) {
    return string
      .replace(/(&lt;\/?)([a-zA-F0-9][\s\S]*?)(&gt;)/g, function(all, tagStart, tagContent, tagEnd) {
        tagContent = tagContent.replace(/^(\S+)/g, '<i>$1</i>');
        tagContent = tagContent.replace(/(\s)([a-zA-Z0-9:_-]+?)(=&quot;)(.*?)(&quot;)/g, '$1<var>$2</var>$3<kbd>$4</kbd>$5');
  ...
  /**
   * Add syntax highlighting to METAR codes
   * @see    https://en.wikipedia.org/wiki/METAR
   * @param  {String} string [description]
   * @return {String}        [description]
   */
  internal.convertMetar = (string) => {
    return string
      .replace(/^\s*((?:METAR|TAF)?\s?\S+)\s(\d+Z)\s(\S+)\s([\s\S]+)$/, (all, header, time, wind, data) => {
        time = time.replace(/(\d{2})(\d{4}Z)/, '<span title="Day of month">$1</span><b title="Time">$2</b>');
        wind = wind.replace(/(\d{3})(.+)/, '<span title="Wind direction">$1</span><b title="Wind speed">$2</b>');
        data = data
          .replace(/((?:CLR|FEW|SCT|BKN|OVC)(?:\d{3})?)\b/g, '<var title="Cloud">$1</var>')
          .replace(/(\d+)\/(\d+)\b/, '<b title="Temperature">$1</b>/<span title="Dewpoint">$1</span>')
          .replace(/([AQ]\d{4})\b/, '<span title="Pressure">$1</span>')
        ;
        return '<i title="Location">' + header + '</i> ' + time + ' ' + wind + ' ' + data;
      })
    ;
  };
    ...
    return html
      ...
      .replace(/(<p(?:\s[^>]+)?>)\s*(<a[^>]+?>([^<]+?)<\/a>)\s*(<\/p>)/g, (all, before, inline, linktext) => { // after
        ...
        return inline
          ...
          .replace(
            /(?:<a)?[^>]*?open.spotify.com\/([^"]+)[^>]*?(?:>(.+?)<\/a>)/g,
            function(all, url) {
              return '<div class="audio-player audio-player--spotify"><iframe title="' + linktext + '" src="https://embed.spotify.com/?uri=' + encodeURIComponent(url) + '"></iframe></div>';
            }
          )
          .replace(
      ...
      .replace(/(<a[^>]+)title="(alternate|external|nofollow|nomention|tag|download)"/g, (all, link, value) => {
        return link + (value === 'download'
          ? value
          : 'rel="' + value + '"'
        );
      })
