      ...
      switch (internal.mode) {
        ...
        case '<>':
          if (chunk.match(/<code class/)) {
            const lang = chunk.match(/(css|less|sass|html|xml|xslt|markdown|shell|bash|dos|yaml|ini|apacheconf|sql|abc|axiom|json)/);
            if (lang && lang[1]) {
              newMode = '<' + lang[1] + '>';
            } else {
              newMode = '<code>';
  ...
  internal.convertCode = function(string) {
    return internal.convertGeneralCode(string, function(codeRest) {
      return codeRest
        ...
        .replace(/(^|\b)(unsigned )?(void|bool(ean)?|int(eger)?|short|float|double|string|char)(\b)/g, '$1<b>$2</b>$3')
        .replace(/(^|\b)(and|array|break|case|die|do|echo|s?printf?|else(if)?|elsif|filter|final|for(each|Each)?|import|join|map|try|catch|then|global|if|import|include(_once)?|length|list|map|new|or|require(_once)?|return|self|sizeof|split|switch|this|throw|while)(\b)/g, '$1<i>$2</i>$3')
  ...
  internal.convertResult = function(html) {
    ...
    return html
      ...
      .replace(/(<p>(?:--|—)[^<]+?<\/p>\s*<blockquote>[\s\S]+?<\/blockquote>\s*)+/g, function(all) {
        ...
        all = all
          .replace(/<p>(?:--|—)\s?([^<]+?)<\/p>\s*(<blockquote>)([\s\S]+?<\/blockquote>)/g, '$2<header>$1</header>$3')
        ;
        all
          .match(/<header>(.+?)<\/header>/g)
      ...
      .replace(/(<blockquote>[^\n]+?)(<\/blockquote>\s*)(<p>\s*(?:--|—) \S[\s\S]+?<\/p>)/g, (all, bqstart, bqend, footer) => {
        // blockquote with appended `--` / source
        return bqstart + footer + bqend;
      })
      .replace(/(<blockquote)(>[^\n]+?\s*)<p>\s*(?:--|—) (\S[\s\S]+?)<\/p>\s*(<\/blockquote>\s*)/g, (all, bqstart, bq, footer, bqend) => {
