...
const blogophonHandlebarsQuoters = {
   ...
   */
  lazyloadAttributes(text, loading = 'lazy', butFirst = false) {
   ...
   * @return {String}               [description]
   */
  niceShorten(text, maxChars = 160, replaceString = '...') {
   ...
   */
  ifEquals(haystack, needle, options) {
   ...
   * @return {Boolean}           [description]
   */
  ifMatches(haystack, pattern, options) {
   ...
   */
  ifHasAspectRatio(imageUrl, width, height, options) {
   ...
   */
  ifHasMinDimensions(imageUrl, width, height, options) {
    width  = Number(width);
   ...
   */
  ifHasMaxDimensions(imageUrl, width, height, options) {
    width  = Number(width);
    height = Number(height);
    const dimensions = blogophonHandlebarsQuoters._getDimensions(imageUrl);
    return (dimensions.width <= width && dimensions.height <= height)
   ...
   */
  ifHasDimensions(imageUrl, options) {
    const dimensions = blogophonHandlebarsQuoters._getDimensions(imageUrl);
...
export default blogophonHandlebarsQuoters;
