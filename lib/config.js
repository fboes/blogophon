...
const config = function(baseDirectory = process.cwd()) {
  ...
  external.generator          = pkg.name + ' ' + pkg.version;
  ...
  external.directories.themeBase = path.join(__dirname, '..', pkg.directories.theme);
  ...
  external.baseUrl            = external.baseUrl       || 'http://' + (require('os').hostname() || 'example.com');
  external.basePath           = external.basePath      || '/';
  external.absoluteBasePath   = external.baseUrl + external.basePath;
  external.domain             = external.baseUrl.replace(/^[a-z]+:\/\//, '');
  ...
  external.locale.direction   = external.locale.direction || (external.locale.language.match(/^(ar|zh|fa|he)/) ? 'rtl' : 'ltr');
  ...
  try {
    external.themeConf = JSON.parse(fs.readFileSync(path.join(external.directories.currentTheme, 'theme.json'), 'utf8'));
    external.themeConf.icons = external.themeConf.icons.map(function(icon) {
      if (!icon.src.match(/^http/)) {
        icon.src = external.absoluteBasePath + external.htdocs.theme + '/' + icon.src;
      }
      return icon;
    });
    if (!external.themeConf.ogImage.match(/^http/)) {
      external.themeConf.ogImage = external.absoluteBasePath + external.htdocs.theme + '/' + external.themeConf.ogImage;
    }
  } catch (e) {
    external.themeConf = {};
  }
