  ...
  external.createArticleDialog = function() {
    ...
    const questions    = [
      {
        ...
        when: function(answers) {
          return answers.classes !== 'Micro post' && blogophonDate(answers.date).iso === blogophonDate(defaults.date).iso;
        }
      }, {
        type: 'confirm',
        name: 'edit',
    ...
    inquirer.prompt(questions).then(
      function(answers) {
        ...
        answers.date = blogophonDate(answers.date).iso || defaults.date;
