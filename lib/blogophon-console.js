  ...
  external.createArticleDialog = function() {
    ...
    const questions    = [
      {
        ...
        when: function(answers) {
          return answers.classes !== 'Micro post' && blogophonDateFormat(answers, 'iso') === blogophonDateFormat(defaults, 'iso');
        }
      }, {
        type: 'confirm',
        name: 'edit',
    ...
    inquirer.prompt(questions).then(
      function(answers) {
        ...
        answers.date = blogophonDateFormat(answers, 'iso') || defaults.date;
