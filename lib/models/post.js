...
const Post = function(filename, markdown, meta, config) {
  ...
  external.makeMeta = function(filename, markdown, meta) {
    ...
    if (meta.Enclosure) {
      if (!Array.isArray(meta.Enclosure)) {
        meta.Enclosure = [meta.Enclosure];
      }
      meta.Enclosure = meta.Enclosure.map(function(e) {
        if (!e.url.match(/^http/)) {
          e.url = meta.AbsoluteUrl + e.url;
        }
        return e;
      });
    }
  ...
  internal.recipeHtml = function(html) {
    return html
      ...
      .replace(/(<ol)(>[\s\S]+?<\/ol>)/, function(all, first, last) {
        return first + ' itemprop="recipeInstructions"' + last;
      })
    ;
  };
