...
// External
const markdownConvert = require('marked');
...
const Post = function(filename, markdown, meta, config) {
  ...
  external.makeMeta = function(filename, markdown, meta) {
    ...
    if (meta.Description) {
      ...
      meta.Description = SuperString(internal.stripHtmlTags(external.htmlTeaser)).niceShorten(320);
    }
    if (!meta.Author) {
    ...
    if (meta.Image) {
      meta.ProperImage = meta.Image; // contains an explcitly set image, ignoring fallback image
    }
  ...
  internal.markyMark = function(markdown, relUrl) {
    let html = markyMark(markdownConvert(markdown), {
    ...
    return html
      ...
      .replace(/(<ol)(>)([\s\S]+?<\/ol>)/, function(all, tagStart, tagEnd, items) {
        return tagStart
          ...
          + tagEnd
          + items.replace(/(<li)/g, '$1 itemprop="itemListElement"');
      })
    ;
  };
