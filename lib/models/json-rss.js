...
const jsonRss = function(index, pubDate, config, title, feedUrl) {
  return {
    ...
    channel: {
      ...
      atomUpdated: blogophonDateFormat(pubDate, 'rfc3339'),
      ...
      items: index.map(function(item){
        let returnItem = {
          ...
          atomPublished: blogophonDateFormat(item.meta.Created, 'rfc3339'),
          atomUpdated: item.meta.Modified ? blogophonDateFormat(item.meta.Modified, 'rfc3339') : null,
